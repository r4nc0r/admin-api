...
def _splitData(args):
    ...
    if "storeprop" in attributes or "remove_storeprop" in attributes:
        cli.print(cli.col("--storeprop and --remove_storeprop arguments are deprecated, use --property and --remove_property "
                          "instead.", "yellow"))
    data["aliases"] = [alias for alias in attributes.pop("alias", []) if alias.strip()] or ()
...
def _usernamesFromFile(args):
    ...
    with Service("exmdb") as exmdb:
        ...
        if args.type == "delegates":
            ...
            content = [member.mail for member in content.members if member.rights & Permissions.FOLDEROWNER]
    return 0, content
...
def _usernamesToFile(usernames, args):
    ...
    with Service("exmdb") as exmdb:
        ...
        if args.type == "delegates":
            ...
            res = client.setFolderMembers(eid, usernames, Permissions.FOLDEROWNER)
            if DB.minVersion(91):
...
def _setupCliUser(subp: ArgumentParser):
    ...
    deviceParser(deviceActions, "remove", cliUserDevicesRemoveResync, help="Remove device from devices list")
    deviceParser(deviceActions, "resync", cliUserDevicesRemoveResync, help="Initiate resynchronization of device")
    deviceWipe = deviceActions.add_parser("wipe", help="Engage device remote wipe")
